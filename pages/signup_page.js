...
export default function SignUp() {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      await fetch("/api/send-welcome-email", {
        ...
        body: JSON.stringify({ email, firstName: preAuthFirstName }),
      });
      ...
      // Create member document with required fields
      await setDoc(doc(db, "members", user.uid), {
        firstName: preAuthFirstName ? formatFirstName(preAuthFirstName) : "",
        uid: user.uid,
        createdAt: new Date(),
        vehicles: [],
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      // Save user profile in Firestore with required fields
      await setDoc(doc(db, "members", user.uid), {
        firstName: firstName,
        uid: user.uid,
        createdAt: new Date(),
        ...
        profileImage: profileImageUrl,
        rating: 5,
        // Optionally keep appleProvider for your logic
        appleProvider: true,
      }, { merge: true });
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4 bg-zinc-900">
      ...
      <div className="relative w-full max-w-2xl flex flex-row bg-white shadow-lg rounded-2xl">
        {/* Cross button for main signup UI */}
        <button
          type="button"
          aria-label="Close"
          className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-2xl"
          ...
          <div className="flex flex-col items-center justify-center w-1/3 p-4 border-r border-gray-200">
            <div className="relative w-32 h-32 rounded-full overflow-hidden shadow-lg bg-gray-100">
              <Image
                ...
                width={128}
                height={128}
                unoptimized={!!(croppedImage || image)}
                priority
              />
              {/* Cropping Modal */}
              {cropping && (
